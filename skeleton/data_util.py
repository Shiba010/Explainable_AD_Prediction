...
class CNN_Data(Dataset):
    ...
    def __init__(self, csv_dir):
        ...
        self.filename, self.label=read_csv(csv_dir)
        self.MRI_data=[]
        self.transform = transforms.Compose([transforms.ToTensor()])

        for MRI_filename in self.filename:
            data=np.load("./ADNI3/"+MRI_filename)  
            data=data[np.newaxis, :, :, :]
            self.MRI_data.append(data)          
        pass
    ...
    def __getitem__(self, idx):
        ...
        '''
        # YOUR CODE HERE 
        return self.MRI_data[idx], self.filename[idx], self.label[idx]
        pass
...
def split_csv(csv_file, output_folder='./ADNI3/', random_seed = 1051):  ## OK
    '''
    Attributes:
    ...
    '''
    # YOUR CODE HERE 
    filepath, label=read_csv(csv_file)
    MRI_idx=[]
    #find 19 MRI file in csv
    for i in range(len(filepath)): 
        if os.access("./ADNI3/"+filepath[i], os.F_OK):
            MRI_idx.append(i)

    test_idx = []
    random.seed(random_seed)
    for i in range(5):
        n = random.randint(0, len(MRI_idx))
        test_idx.append(n)
    ...
    bg_file=[]
    bg_label=[]
    for i in range(len(MRI_idx)):
        if i in test_idx:
            test_file.append(filepath[MRI_idx[i]])  
            test_label.append(label[MRI_idx[i]])           
        else:
            bg_file.append(filepath[MRI_idx[i]])  
            bg_label.append(label[MRI_idx[i]]) 

    test_data={"filename":test_file, "AD":test_label}
    df1=pd.DataFrame(test_data)   
    df1.to_csv(output_folder+"Test data.csv", index=False)
    bg_data={"filename":bg_file, "AD":bg_label}
    df2=pd.DataFrame(bg_data)  
    df2.to_csv(output_folder+"Background data.csv", index=False)

    #return "Test data.csv", "Background data.csv"
    pass
